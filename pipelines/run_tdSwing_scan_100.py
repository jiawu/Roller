...
def main(data_folder, output_path, target_dataset, my_iterating_param, param_test_style, param_tests, n_trials):
    ...
    default_params = {'data_folder':data_folder, 'file_path':target_dataset, 'td_window':21,'min_lag':1,'max_lag':3,'n_trees':500,'permutation_n':5, 'lag_method':'mean_mean', 'calc_mse':False, 'bootstrap_n':50,'n_trials':n_trials, 'run_time':current_time, 'sort_by': 'rank','iterating_param':my_iterating_param, 'filter_noisy':False, 'alpha': None}
    ...
    for current_param_value in param_tests:
        for trial in range(0,n_trials):
            ...
            if (run_params['td_window'] == 6) or (('_sampling' in run_params['file_path']) and run_params['td_window'] == 7) or (('dream8/insilico' in run_params['file_path']) and run_params['td_window'] == 11) or(('dream8/invitro' in run_params['file_path']) and run_params['td_window'] == 7):
...
if __name__ == "__main__":
    ...
    elif param_test_style == "float":
        param_tests = [float(x) for x in sys.argv[6:]]
    ...
    elif param_test_style == "float-minmax":
        param_min = float(sys.argv[6])
        param_max = float(sys.argv[7])
        param_step_size = float(sys.argv[8])
        param_tests = list(np.arange(param_min,param_max, param_step_size))
