...
def main(window_type='RandomForest', CLUSTER=4):
    ...
    for clusterid in clusters:
        ...
        if (len(current_group) < 10) or (len(sub_dict['tfs']) < 2):
            continue
        pr1,roc1 = run_subswing(current_group, td_window = 6, min_lag = 0, max_lag = 0, window_type = window_type, clusterid = clusterid, output_fn = output_file, tfs = lagged_TFs, alpha = .30, n_trees=10)
        pr2,roc2 = run_subswing(current_group, td_window = 5, min_lag = 1, max_lag = 1, window_type = window_type, clusterid = clusterid, output_fn = output_file2, tfs = lagged_TFs, alpha = .30, n_trees=10, pcs=4)
        pr3,roc3 = run_subswing(current_group, td_window = 5, min_lag = 0, max_lag = 1, window_type = window_type, clusterid = clusterid, output_fn = output_file3, tfs = lagged_TFs, n_trees=10)
        pr4,roc4 = run_subswing(current_group, td_window = 4, min_lag = 1, max_lag = 2, window_type = window_type, clusterid = clusterid, output_fn = output_file4, tfs = lagged_TFs, n_trees=10)
    ...
    current_time = datetime.now().strftime('%Y-%m-%d_%H:%M:%S')    
    cluster_summary.to_csv('cluster_summaries_tf/sc_cluster_summary_within_community_c'+str(CLUSTER)+'_' + current_time + '.csv', header=True, index=False, sep='\t')
