...
def generate_json(merged_lag,method, CLUSTER=14):
    ...
    with open('yeast_lagged_network_c'+str(CLUSTER)+'.json', 'w') as fp:
...
def main(window_type='RandomForest', CLUSTER=14):
    ...
    parsed_path = '../../data/invitro/yeast_cluster_assignments_parsed'+str(CLUSTER)+'.csv'
...
marbach_promotion = pd.read_pickle('/projects/p20519/roller_output/pickles/marbach_promotion.pkl')

promotion_lag = pd.merge(merged_lag, marbach_promotion, how='left', left_on=['index'], right_on=['regulator-target'])
