...
def run_subswing(df, td_window=6, min_lag = 0, max_lag = 0, window_type = 'RandomForest', clusterid = None, output_fn = None, tfs = None, alpha = None, pcs = None, n_trees = 1000):
    ...
    for window_type in window_types:
        ...
        tdr.rank_edges(permutation_n=500, n_bootstraps=500)
    ...
    sub_df=sub_df.merge(df,how = 'left', right_on='Edge',left_on='regulator-target')
...
def main(window_type='RandomForest', CLUSTER=26):
    ...
    output_file = "networks/community_omranian_clusters_" + current_time + ".tsv"
    output_file2 = "networks/SWING_community_omranian_clusters_" + current_time + ".tsv"
    output_file3 = "networks/SWING2_community_omranian_clusters_" + current_time + ".tsv"
    output_file4 = "networks/SWING3_community_omranian_clusters_" + current_time + ".tsv"
    
    for clusterid in clusters:
        print(clusterid, len(clusters))
        current_group = grouped_by_cluster.get_group(clusterid)
        lagged_TFs = None
        current_group = merged_lag[(merged_lag['parent_cluster'] == clusterid) | (merged_lag['child_cluster'] == clusterid)]
        ...
        if (len(current_group) < 10) or (len(sub_dict['tfs']) < 2):
            continue
    
        pr1,roc1 = run_subswing(current_group, td_window = 5, min_lag = 0, max_lag = 0, window_type = window_type, clusterid = clusterid, output_fn = output_file, tfs = lagged_TFs, alpha = .30, n_trees=10)
        pr2,roc2 = run_subswing(current_group, td_window = 4, min_lag = 1, max_lag = 1, window_type = window_type, clusterid = clusterid, output_fn = output_file2, tfs = lagged_TFs, alpha = .30, n_trees=10, pcs=4)
        pr3,roc3 = run_subswing(current_group, td_window = 4, min_lag = 0, max_lag = 1, window_type = window_type, clusterid = clusterid, output_fn = output_file3, tfs = lagged_TFs, n_trees=10)
        pr4,roc4 = run_subswing(current_group, td_window = 3, min_lag = 1, max_lag = 2, window_type = window_type, clusterid = clusterid, output_fn = output_file4, tfs = lagged_TFs, n_trees = 10)
    ...
    current_time = datetime.now().strftime('%Y-%m-%d_%H:%M:%S')    
    cluster_summary.to_csv('cluster_summaries_tf/cluster_summary_within_community_c'+str(CLUSTER)+'_' + current_time + '.csv', header=True, index=False, sep='\t')
