...
class GnwWrapper(object):
    ...
    def simulate(self, network_file, settings=None):
        if settings is None:
            settings = self.settings_default

        p = subprocess.Popen(self.jar_call + ['--simulate', '-c', settings, '--input-net', network_file])
...
if __name__ == '__main__':
    ...
    • The script assumes that the xml files already exist. These came from the previous results testing.
    
    """

    directory = '/Users/jfinkle/Documents/Northwestern/MoDyLS/Code/Python/Roller/data/gnw_insilico/high_sampling/'
    jar_location = '/Users/jfinkle/Documents/Northwestern/MoDyLS/Code/gnw/'
    ...
    for bug in network:
        for n in n_nodes:
            data_out = '{}{}{}/'.format(directory, bug, n)
            os.chdir(data_out)
            ...
            for net in range(1, num_nets+1):
                if n != 10:
                    ...
                    appendix = ''
                infile = '{}/{}-{}.xml'.format(data_out, bug+appendix, net)
                if os.path.isfile(infile.replace('.xml', '_goldstandard.tsv')):
                    continue
                print(infile)
                gnw.simulate(infile, sim_settings)
